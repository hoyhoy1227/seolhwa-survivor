...
def process(source: Path) -> Path:
    ...
    padding = max(18, round(side * 0.08))
    ...
    output_name = source.name.replace("-alpha", "")
...
if __name__ == "__main__":
    files = sorted(SOURCE_DIR.glob("boss-*-alpha.png"))
    if not files:
        raise SystemExit("No boss alpha sprites found")
    for file in files:
        print(process(file))
